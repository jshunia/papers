...
Let \( n \) be an odd composite integer greater than $3$ such that \( 2^{n-1} \equiv 1 \pmod{n} \). Denote by \( D \) the smallest integer \(2 < D < n \) which does not divide \( n-1 \). Then, there exists at least one integer $k$ in $1 < k < n$ for which $\binom{n}{k} \not\equiv 0 \pmod{n}$, $2^{\left\lfloor \frac{n-1}{D} \right\rfloor k} \not\equiv 2^{\left\lfloor \frac{k}{D} \right\rfloor} \pmod{n}$, and $\binom{n}{k} \left(2^{\left\lfloor \frac{n-1}{D} \right\rfloor k} - 2^{\left\lfloor \frac{k}{D} \right\rfloor}\right) \not\equiv 0 \pmod{n}$.
\end{lemma}
\begin{proof}
Let \( p \) be a prime factor of \( n \). We will examine the cases \( k = p \) and \( k = n - p \).

As shown in \cite{ogilvy1956binomial}, for any prime \( p \) dividing \( n \), we have \( \binom{n}{p} \not\equiv 0 \pmod{n} \). Due to the inherit symmetry of binomial coefficients for $n$ around the term at $k = \left\lfloor\frac{n}{2}\right\rfloor + (n \bmod{2})$, we have:
\begin{align}
\binom{n}{n-p} = \binom{n}{p} \not\equiv 0 \pmod{n}
\end{align}

First, we briefly examine the exponents at $k=p$ and $k=n-p$:
\begin{itemize}
    \item For \( k = p \), we have \( 2^{\left\lfloor \frac{n-1}{D} \right\rfloor p} \) and \( 2^{\left\lfloor \frac{p}{D} \right\rfloor} \).
    \item For \( k = n - p \), we have \( 2^{\left\lfloor \frac{n-1}{D} \right\rfloor (n - p)} \) and \( 2^{\left\lfloor \frac{n-p}{D} \right\rfloor} \).
\end{itemize}

Now, let $k=p$. Assume that $p$ satisfies:
\begin{align}
    2^{\left\lfloor \frac{n-1}{D} \right\rfloor p} - 2^{\left\lfloor \frac{p}{D} \right\rfloor} \equiv 0 \pmod{n}
\end{align}

Regarding $\text{ord}_n(2)$, this implies: 
\begin{align}
    \left\lfloor \frac{n-1}{D} \right\rfloor p - \left\lfloor \frac{p}{D} \right\rfloor \equiv 0 \pmod{\text{ord}_n(2)} 
\end{align}

Now, let's examine what it means for $k=n-p$. Recall of course that $p$ and $n-p$ are both odd and $D > 2$. Consequently, the rounding down which occurs during the flooring of $\left\lfloor \frac{n-p}{D} \right\rfloor$ causes an off-by-one error to accumulate in the difference of the exponents modulo $\text{ord}_n(2)$, perturbing the congruence. As a result of this action, we must have:
\begin{align}
    \left\lfloor \frac{n-1}{D} \right\rfloor (n-p) - \left\lfloor \frac{n-p}{D} \right\rfloor \equiv -1 \pmod{\text{ord}_n(2)} 
\end{align}

Which means $2^{\left\lfloor \frac{n-1}{D} \right\rfloor (n-p)} \equiv 1 \pmod{n}$ and $2^{\left\lfloor \frac{n-p}{D} \right\rfloor} \equiv 2^{-1} \pmod{n}$, leading to:
\begin{align}
    2^{\left\lfloor \frac{n-1}{D} \right\rfloor (n-p)} - 2^{\left\lfloor \frac{n-p}{D} \right\rfloor} \equiv 2^{-1} \pmod{n}
\end{align}
...
Further, since $\binom{n}{k}$ is non-zero, we may also conclude:
\begin{align}
\binom{n}{k} \left(2^{\left\lfloor \frac{n-1}{D} \right\rfloor k} - 2^{\left\lfloor \frac{k}{D} \right\rfloor}\right) \not\equiv 0 \pmod{n}
\end{align}

Finally, given the symmetry of binomial coefficients as described earlier, this proof also applies in the case that $2^{\left\lfloor \frac{n-1}{D} \right\rfloor (n-p)} - 2^{\left\lfloor \frac{n-p}{D} \right\rfloor} \equiv 0 \pmod{n}$ by swapping the exponents $k=p$ and $k=n-p$.
