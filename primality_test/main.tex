...
Let $n = pq$ be an odd composite integer $>3$ with $p$ a prime divisor. Let $d > 2 \in \mathbb{Z}$ be the prime integer such that $n \not\equiv 1 \pmod{d}$. Let $x^d = 2$, and suppose $x^n = 2^{\left\lfloor\frac{n-1}{d}\right\rfloor} \not\equiv 1 \pmod{n}$. Consider the polynomial $f(x) = (1 + x)^n - (1 + x^n) \in \mathbb{Z}_n[x]$. If $n$ does not have a prime divisor $\leq d$, then $f(x)$ is nonzero when reduced modulo $x^d - 2$. That is, we must have $(1 + x)^n \not\equiv 1 + x^n \pmod{n, x^d-2}$.
\end{theorem}
\begin{proof}
We are given composite $n$ with $2^{\left\lfloor\frac{n-1}{d}\right\rfloor} \not\equiv 1 \pmod{n}$.

If $f(x) = (1 + x)^n - (1 + x^n)$ is to be zero modulo $n$, it is necessary but not sufficient that $f(x)$ be zero modulo all of the individual prime factors of $n$. This is because if any of the polynomial's coefficients are indivisible by some $p$ dividing $n$, then they cannot be divisible by $n$. We will prove that under the given conditions, there must exist a prime divisor $p$ of $n$ such that $f(x) = (1 + x)^n - (1 + x^n) \in \mathbb{Z}_p[x]$ is nonzero when reduced modulo $x^d - 2$.
...
\begin{enumerate}
    ...
    \item \textbf{Computing \( (1+x)^n \bmod{n, x^d-2} \)}:
        ...
        Computing the polynomial expansion of \( (1+x)^n \pmod{n} \) in the ring \( (\Zn)[x]/(x^d-2) \)  with degree $d$ involves exponentiating a polynomial in $(\Zn)[x]/(x^d-2)$ with \( D = O((1 + o(1))\log(n)) \) terms. Exponentiation using repeated squaring takes \( O(\log(n)) \) steps, and each step requires \( O((1 + o(1)) \log(n) M(n)) \) time due to the multiplication of polynomials of size \( O((1 + o(1)) \log(n)) \). Therefore, the overall complexity of this step is \(  T_4(n) =O((1 + o(1)) \log^2(n) M(n)) \).
...
The dominant time complexity in the algorithm comes from computing \( (1+x)^n \pmod{n} \in (\Zn)[x]/(x^d-2) \). Therefore, the overall time complexity of the algorithm is \( T(n) = O((1 + o(1)) \log^2(n) M(n)) \).
