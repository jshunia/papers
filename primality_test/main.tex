...
Now, suppose cases (i) and (ii) are both false. The polynomial generator $x^r-2$ factors as $(x-\alpha)^r \pmod{p}$, where $\alpha$ is the root of $x^r-2$ in an algebraic closure of $\mathbb{F}_p$. Since $n$ is a Carmichael number and $r \nmid (n-1)$, it follows that for all $p \mid n$, we have $r \nmid (p-1)$ and thus $\alpha \equiv 2^{r^{-1} \bmod (p-1)} \pmod{p}$.
...
\end{align*}
The ring $\mathbb{F}_p[x]/((x-\alpha)^r)$ is Artinian, with a chain of ideals $(x-\alpha)^1 \supset (x-\alpha)^2 \supset \cdots \supset (x-\alpha)^r$. Furthermore, since $(x-\alpha)^r \equiv 0 \pmod{p}$, it is a nilpotent ring. In this context, $f(x)$ is trivially zero in $\mathbb{F}_p[x]/((x-\alpha)^r)$. Then, to be zero in the ring $\Z[x]/(x^r-2, p)$, the remainder $f(x) \pmod{p}$ must be in the ideal generated by $(x^r-2)(x-\alpha)^r$. In the ring $\mathbb{F}_p[x]/((x^r-2)(x-\alpha)^r)$, there may exist non-trivial $p$-th roots of unity, such that
\begin{align*}
...
\end{align*}
Lifting the congruence mod $n$ via the CRT, we see
\begin{align*}
    \zeta_n \equiv (x+1)^n - x^n \not\equiv 1 \pmod{((x^r-2)(x-\alpha)^r, n)} .
\end{align*}
This implies that
\begin{align*}
f(x) \not\equiv 0 \pmod{((x^r-2)(x-\alpha)^r, n)} .
\end{align*}
Now, consider the natural projection homomorphism:
\begin{align*}
\phi : \Z[x]/((x^r-2)(x-\alpha)^r, n) \to \Z[x]/(x^r-2, n) .
\end{align*}
Applying $\phi$ to both sides of the non-congruence, we get
\begin{align*}
\phi(f(x)) \not\equiv 0 \pmod{(x^r-2, n)} .
\end{align*}
But $\phi(f(x)) = f(x)$, so we have
\begin{align*}
f(x) \not\equiv 0 \pmod{(x^r-2, n)} .
\end{align*}
Therefore, if cases (i) and (ii) are both false for some $p \mid n$, then $f(x) \not\equiv 0 \pmod{(x^r-2, n)}$, contradicting the theorem.
