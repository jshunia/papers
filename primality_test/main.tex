...
Now, suppose cases (i) and (ii) are both false. If $x^n \equiv \zeta_p x \pmod{(x^r-2, p)}$, where $\zeta_p$ is a non-trivial $p$-th root of unity modulo $(x^r-2, p)$, then $(x+1)^n \equiv \zeta_p (x+1) \pmod{(x^r-2, p)}$. This is possible because the polynomial ring $\Z[x]/(x^r-2, p)$, the polynomial generator $x^r-2$ factors as $(x-\alpha)^r \pmod{p}$. Thus, the ring is isomorphic to the direct product of field taken $r$ times $\mathbb{F}_p[x]/(x-\alpha) \times \cdots \times \mathbb{F}_p[x]/(x-\alpha)$, where  $\alpha$ is the root of $x^r-2$ in an algebraic closure of $\mathbb{F}_p$. In particular, $\Z[x]/(x^r-2, p) \cong \mathbb{F}_p[x]/((x-\alpha)^r)$. In the field $\mathbb{F}_p[x]/(x-\alpha)$, there may exist non-trivial $p$-th roots of unity, allowing for this. However, we showed above that $\zeta_n \equiv 1 \pmod{(x^r-2, n)}$, so this would imply $x^n \equiv \zeta_n x \equiv x \pmod{(x^r-2, n)}$, contradicting the assumption in the theorem that $x^n \not\equiv x \pmod{(x^r-2, n)}$.
...
For $n$, again lifting via the CRT, since $n$ is a Carmichael number and $(p-1) \mid (n-1)$ for all $p$, this would imply
\begin{align*}
    2^{\floor{n/r}} \equiv 1 \pmod{n} 
    \quad \Longleftrightarrow \quad
    \floor{n/r} \equiv 0 \pmod{\ord_2 n} ,
\end{align*}
where $\ord_2 n$ is the multiplicative order of $2$ modulo $n$. However, since $n$ is a Carmichael number, this is impossible, since $\ord_2 n \mid (n-1)$, and $r \nmid (n-1)$.
...
It is tempting to wonder if perhaps $\tilde{O}(\log^3 n)$ is the end of the road for primality testing. However, it the opinion of the author that this is likely not the case. As Agrawal did in the PRIMES is in P paper \cite{aks2002}, we conclude with a conjecture on primality.
\begin{conjecture}
Let $n \in \mathbb{Z}_{\mathrm{odd}}^+$ such that $2^{n-1} \equiv 1 \pmod{n}$. Suppose
\begin{align*}
    (x^4 + x)^n - x^{4n} - x^n \equiv 0 \pmod{(x^8 - x^2 + 2, n)} .
\end{align*}
Then, $n$ is prime.
\end{conjecture}
The time complexity of the conjectured primality test is $\tilde{O}(\log^2 n)$. There are no counterexamples to the conjecture for $n \leq 2^{64}$. While we believe this particular version of the conjecture is likely false, as with Agrawal's conjecture, it it is plausible that a variation can be constructed which is indeed infallible.
