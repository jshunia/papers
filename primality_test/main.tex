...
\end{lemma}
\begin{proof}
Consider an odd composite integer \( n > 3 \) for which \( 2^{n-1} \equiv 1 \pmod{n} \). According to the properties of the multiplicative order modulo $n$, the smallest positive integer \( k \) such that \( 2^k \equiv 1 \pmod{n} \) defines \( \text{ord}_n(2) \), i.e., \( k = \text{ord}_n(2) \). Since $n$ is composite and \( 2^{n-1} \equiv 1 \pmod{n} \), this order, \( \text{ord}_n(2) \), must divide \( n-1 \).
...
Let \( n \) be an odd composite integer greater than $3$ such that \( 2^{n-1} \equiv 1 \pmod{n} \). Denote by \( D \) the smallest integer \(2 < D < n \) which does not divide \( n-1 \). Then, there exists at least one integer $k$ in $1 < k < n$ for which $\binom{n}{k} \not\equiv 0 \pmod{n}$ and $2^{\left\lfloor \frac{n-1}{D} \right\rfloor k} \not\equiv 2^{\left\lfloor \frac{k}{D} \right\rfloor} \pmod{n}$.
\end{lemma}
\begin{proof}
\textbf{NOTE: THIS PROOF IS INCORRECT! I need to fix it.}
Let $f(x) = 2^{\left\lfloor \frac{x-1}{D} \right\rfloor}$. Hence, we have: $f(n)^k = 2^{\left\lfloor \frac{n-1}{D} \right\rfloor k}$, $f(k+1) = 2^{\left\lfloor \frac{k}{D} \right\rfloor}$.
...
\begin{align} \label{equation:kcongruence}
...
We choose $p$ to be a prime factor of $n$. As shown by Ogilvy (1956) \cite{ogilvy1956binomial}:
\begin{align}
\binom{n}{p} \not\equiv 0 \pmod{n}
\end{align}

Denote $\varphi(n)$ as the valuation of Euler's totient function for $n$. Given $p \mid n$, we have $p-1 \mid \varphi(n)$. Therefore, $p \nmid \varphi(n)$ and thus $p \nmid \text{ord}_n(2)$. We now examine the exponents of $f(n)^k$ and $f(k+1)$ in the case that $k=p$. Recall \cref{equation:kcongruence}. Translating this to our current situation, in order for $f(n)^p \equiv f(p+1) \pmod{n}$ to hold, we require:
\begin{align}
    \left\lfloor\frac{n-1}{D}\right\rfloor p \equiv \left\lfloor\frac{p}{D}\right\rfloor \pmod{\varphi(n) \text{, } \text{ord}_n(2)}
\end{align}

Given $p$ is prime, it is obvious that $\frac{p}{D}$ cannot be an integer and also that $\left\lfloor\frac{p}{D}\right\rfloor \nmid p$. Further, since $n$ is odd, $p$ must be odd. It follows that $\left\lfloor\frac{p}{D}\right\rfloor = \left\lfloor\frac{p-1}{D}\right\rfloor$. As such, by Lemma \ref{lemma:floornondivisor} we may say that $\left\lfloor\frac{p-1}{D}\right\rfloor \nmid p-1$. While we know $p-1 \mid \text{ord}_n(2)$, $\left\lfloor\frac{p-1}{D}\right\rfloor \nmid p-1$ does not imply $p(k+1) \not\equiv 1 \pmod{n}$, as it is still possible for $\left\lfloor\frac{p}{D}\right\rfloor$ to be equal to $\text{ord}_n(2)$ or a multiple of $\text{ord}_n(2)$.

%And since $p-1 \mid \text{ord}_n(2)$, by Lemma \ref{lemma:orderinequality} we have $f(p+1) \not\equiv 1 \pmod{n}$ unless $p \leq D$ or $\left\lfloor\frac{p}{D}\right\rfloor = $.

Conversely, $\left\lfloor\frac{n-1}{D}\right\rfloor p$ is a multiple of a prime $p$. Since we know ${\left\lfloor\frac{n-1}{D}\right\rfloor} \not= \text{ord}_n(2)$, it follows that ${\left\lfloor\frac{n-1}{D}\right\rfloor p} \not= \text{ord}_n(2)$ and $f(n)^p = 2^{\left\lfloor\frac{n-1}{D}\right\rfloor p} \not\equiv 1 \pmod{n}$.

Based on the above, it is plausible for $\left\lfloor\frac{n-1}{D}\right\rfloor p \equiv \left\lfloor\frac{p}{D}\right\rfloor \pmod{\text{ord}_n(2)}$ to hold for a given prime factor $p$ of $n$ in isolation. However, for our purposes it must hold for all prime factors of $n$. Let's examine the case where $n$ is the product of two prime factors $n = p \cdot q$. We must have:
\begin{align}
\left\lfloor\frac{n-1}{D}\right\rfloor p - \left\lfloor\frac{p}{D}\right\rfloor \equiv \left\lfloor\frac{n-1}{D}\right\rfloor q - \left\lfloor\frac{q}{D}\right\rfloor \equiv 0 \pmod{\text{ord}_n(2)}
\end{align}

Rewriting this:
\begin{align} \label{equation:pqcongruence}
\left\lfloor\frac{n-1}{D}\right\rfloor p - \left\lfloor\frac{n-1}{D}\right\rfloor q \equiv \left\lfloor\frac{p}{D}\right\rfloor - \left\lfloor\frac{q}{D}\right\rfloor \equiv 0 \pmod{\text{ord}_n(2)}
\end{align}

We focus on the left-hand side:
\begin{align}
\left\lfloor\frac{n-1}{D}\right\rfloor p - \left\lfloor\frac{n-1}{D}\right\rfloor q \equiv 0 \pmod{\text{ord}_n(2)}
\end{align}

We know $p,q \nmid \text{ord}_n(2)$ and $\left\lfloor\frac{n-1}{D}\right\rfloor \not= \pmod{\text{ord}_n(2)}$. This implies that the difference $\left\lfloor\frac{n-1}{D}\right\rfloor p - \left\lfloor\frac{n-1}{D}\right\rfloor q$ is not equal to $\text{ord}_n(2)$, and further, that it it is not a multiple of $\text{ord}_n(2)$. Thus:
\begin{align}
\left\lfloor\frac{n-1}{D}\right\rfloor p - \left\lfloor\frac{n-1}{D}\right\rfloor q \not\equiv 0 \pmod{\text{ord}_n(2)}
\end{align}

Which gives us a contradiction in \cref{equation:pqcongruence}. Consequently, we may conclude that for at least $1$ prime divisor $k = p_i$ of composite \( n = p_1^{e_1} p_2^{e_2} \ldots p_i^{e_r} \), we have $\binom{n}{k} \not\equiv 0 \pmod{n}$ and $2^{\left\lfloor \frac{n-1}{D} \right\rfloor k} \not\equiv 2^{\left\lfloor \frac{k}{D} \right\rfloor} \pmod{n}$. This completes the proof.
\end{proof}
