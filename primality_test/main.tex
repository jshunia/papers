...
\section{Efficient Computations Using Polynomial Rings}
In this section, we define a special polynomial ring $R$ with a modular variation $M$, and show how $M$ can be used to efficiently compute the value of $\sum_{k=0}^{n} \binom{n}{k} 2^{\left\lfloor \frac{k}{D} \right\rfloor} \pmod{n}$.
...
Define the ring $R$ as defined by Definition \ref{definition:ring} with $N = 2$, $d = 3$. Hence, $\Phi(x^3) = 2$. $P(x) := 1 + x + 3x^3 + x^{4} + x^{6}, P(x) \in R$.
\begin{align}
...
\subsection{Ring Computations}
We now demonstrate how our modular polynomial ring $M$ can be used to efficiently compute the value of $\sum_{k=0}^{n} \binom{n}{k} 2^{k+1} \pmod{n}$.
...
Let $n$, $k$ be non-negative integers. Let $D$ be an integer $>2$. Define the ring $M$ as in Definition \ref{definition:modularring} with $d=D-1$, $N=2$, hence $\Phi(x^{D-1}) = 2$. $P(x) := x, P(x) \in M$. Taking the sum of the coefficients modulo $n$ in the polynomial expansion of \( P(x)^k \), gives $2^{\left\lfloor \frac{k-1}{D} \right\rfloor} \pmod{n}$.
\end{theorem}
\begin{proof}
Examining $D = 2$, we can see:
\begin{align*}
    P(x)^0 = 2^{\left\lfloor -1/2 \right\rfloor} = 2^0 = 1 \\
    P(x)^1 = 2^{\left\lfloor 0/2 \right\rfloor} = 2^0 = 1 \\
    P(x)^2 = 2^{\left\lfloor 1/2 \right\rfloor} = 2^0 = 1 \\
    P(x)^3 = 2^{\left\lfloor 2/2 \right\rfloor} = 2^1 = 2 \\
    P(x)^4 = 2^{\left\lfloor 3/2 \right\rfloor} = 2^1 = 2 \\
    P(x)^5 = 2^{\left\lfloor 4/2 \right\rfloor} = 2^2 = 4 \\
    P(x)^6 = 2^{\left\lfloor 5/2 \right\rfloor} = 2^2 = 4 \\
    \vdots
\end{align*}
...
Assume the theorem holds for some \( k \geq 0 \), i.e., the sum of coefficients modulo \( n \) in \( P(x)^k \) is \( 2^{\left\lfloor \frac{k-1}{D} \right\rfloor} \pmod{n} \). We need to show it also holds for \( k+1 \).

Consider \( P(x)^{k+1} = P(x)^k \ast P(x) \). By the definition of \( \ast \) and \( \Phi \), the degree of \( P(x)^{k+1} \) gets reduced by \( \Phi \) every time it reaches \( D \). The number of times this reduction happens is \( \left\lfloor \frac{k}{D} \right\rfloor \). Therefore, the sum of coefficients in \( P(x)^{k+1} \) should be \( 2^{\left\lfloor \frac{k}{D} \right\rfloor} \), as each reduction by \( \Phi \) doubles the sum of coefficients. Hence, the theorem holds for \( k+1 \).
...
\begin{align}
    (1 + x)^n = \sum_{k=0}^{n} \binom{n}{k} P(x)^{k+1}
\end{align}
Hence, evaluating the expanded polynomial $(1+P(x))^n = (1+x)^n$ at \( x=1 \) yields the binomial transform of the sequence generated by \( P(x)^k \). This follows from the design of our polynomial ring. The remainder of the proof follows from Theorem \ref{theorem:2}, which shows that the sum of coefficients in the expansion of $P(x)^{k+1}$ is equivalent to $2^{\left\lfloor \frac{k}{D} \right\rfloor} \pmod{n}$. 
\end{proof}
