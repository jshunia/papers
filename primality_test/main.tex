...
Now, suppose cases (i) and (ii) are both false. The polynomial generator $x^r-2$ factors as $(x-\alpha)^r \pmod{p}$, where  $\alpha$ is the root of $x^r-2$ in an algebraic closure of $\mathbb{F}_p$. Thus, the ring is isomorphic to the direct product of the field taken $r$ times $\mathbb{F}_p[x]/(x-\alpha) \times \cdots \times \mathbb{F}_p[x]/(x-\alpha)$. In particular, $\Z[x]/(x^r-2, p) \cong \mathbb{F}_p[x]/((x-\alpha)^r)$. Since $f(x)$ is trivially zero in $\mathbb{F}_p[x]/((x-\alpha)^r)$, to be zero in the ring $\Z[x]/(x^r-2, p)$, the remainder $f(x) \pmod{p}$ must be in the ideal generated by $(x^r-2)(x-\alpha)^r$. In the ring $\mathbb{F}_p[x]/((x^r-2)(x-\alpha)^r)$, there may exist non-trivial $p$-th roots of unity, such that
\begin{align*}
...
\begin{align*}
    x^n &\equiv \zeta_n x \pmod{((x^r-2)(x-\alpha)^r, n)} \\
    &\equiv ((x+1)^{n/n} - x^{n/n}) x \pmod{((x^r-2)(x-\alpha)^r, n)} \\
    &\equiv (x+1 - x) x \pmod{((x^r-2)(x-\alpha)^r, n)} \\
    &\equiv x \pmod{((x^r-2)(x-\alpha)^r, n)} .
\end{align*}
Now, consider the natural projection homomorphism:
\begin{align*}
    \phi : \Z[x]/((x^r-2)(x-\alpha)^r, n) \to \Z[x]/(x^r-2, n) .
\end{align*}
Applying $\phi$ to both sides of the congruence, we get
\begin{align*}
    \phi(x^n) \equiv \phi(x) \pmod{(x^r-2, n)}
    \Longleftrightarrow x^n 
    \equiv x \pmod{(x^r-2, n)} .
\end{align*}
However, this contradicts the assumption in the theorem that $x^n \not\equiv x \pmod{(x^r-2, n)}$. Therefore, the existence of non-trivial $p$-th roots of unity in the ring $\mathbb{F}_p[x]/((x^r-2)(x-\alpha)^r)$ that satisfy the given congruences would lead to a contradiction. Thus, we conclude that if cases (i) and (ii) are both false, then $f(x) \not\equiv 0 \pmod{(x^r-2, p)}$ for some prime factor $p$ of $n$, which implies $f(x) \not\equiv 0 \pmod{(x^r-2, n)}$.
