...
\end{proof}
\begin{corollary} \label{proof:leastcoprimesofto}
...
\begin{enumerate}
    ...
    \item \textbf{Finding $r$}:
        ...
        Finding the least $r \in \mathbb{P}_{\geq 3}$ such that $n \not\equiv 1 \pmod{r}$ takes at most $O(\log n (\log \log n + \log \log \log n))$ steps (\cref{proof:leastcoprime}), with each step requiring $O(1)$ time for the mod operation. Hence, the overall complexity of this step is $ T_2(n) = O(\log n (\log \log n + \log \log \log n))$.
