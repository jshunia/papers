...
\begin{theorem}
\label{theorem:1}
Let $n$ be an odd integer $> 3$ satisfying $2^{n-1} \equiv 1 \pmod{n}$. Denote $D$ as the least integer greater than $2$ and less than $n$ which does not divide $n-1$. If $n$ does not have a prime factor $\leq D+1$, then $n$ is prime if and only if the following congruence holds:
\begin{align}
...
Let \( n \) be an odd composite integer greater than $3$ such that \( 2^{n-1} \equiv 1 \pmod{n} \). Denote by \( D \) the smallest integer \(2 < D < n \) which does not divide \( n-1 \). If $n$ does not have a prime factor $p$ such that $p \leq D+1$, then there exists at least one integer $k$ in $1 < k < n$ for which $\binom{n}{k} \not\equiv 0 \pmod{n}$ and $2^{\left\lfloor \frac{n-1}{D} \right\rfloor k} \not\equiv 2^{\left\lfloor \frac{k}{D} \right\rfloor} \pmod{n}$.
\end{lemma}
\begin{proof}
...
\begin{align}
    \left\lfloor \frac{n-1}{D} \right\rfloor p \equiv \left\lfloor \frac{p}{D} \right\rfloor \pmod{\text{ord}_n(2)} 
\end{align}

Since we know $D$ is the least non-divisor of $n-1$, and $n$ does not have a prime factor $\leq D+1$ (hence $\gcd(D, \ordn{2}) = 1$), we can assume that $D^{-1}$ exists modulo $\ordn{2}$. Using a well known identity at for the floor function $\floor{\frac{a}{b}} = b^{-1} \cdot (a - (a \bmod{b}))$, simplifying gives:
\begin{align}
    \left\lfloor \frac{n-1}{D} \right\rfloor p \equiv \left\lfloor \frac{p}{D} \right\rfloor \pmod{\text{ord}_n(2)} \\
    D^{-1} \cdot (n - 1 - ((n-1) \bmod{D})) - D^{-1} \cdot (p - (p \bmod{D})) \equiv 0 \pmod{\ordn{2}} \\
    D^{-1} \cdot (n - 1 - ((n-1) \bmod{D})) \equiv D^{-1} \cdot (p - (p \bmod{D})) \pmod{\ordn{2}} \\
    ...
    p \equiv (p \bmod{D}) - ((n-1) \bmod{D})) + n - 1  \pmod{\ordn{2}}
\end{align}'

Comparing to the simplification at $k=n-p$:
\begin{align}
    \left\lfloor \frac{n-1}{D} \right\rfloor (n-p) \equiv \left\lfloor \frac{n-p}{D} \right\rfloor \pmod{\text{ord}_n(2)} \\
    D^{-1} \cdot (n - 1 - ((n-1) \bmod{D})) - D^{-1} \cdot (n - p - ((n-p) \bmod{D})) \equiv 0 \pmod{\ordn{2}} \\
    D^{-1} \cdot (n - 1 - ((n-1) \bmod{D})) \equiv D^{-1} \cdot (n - p - ((n-p) \bmod{D})) \pmod{\ordn{2}} \\
    ...
    p \equiv ((n-p) \bmod{D}) - ((n-1) \bmod{D})) - 1 \pmod{\ordn{2}}
\end{align}

Due to the equivalence of the above congruences at $k=p$ and $k=n-p$, we must have:
\begin{align}
    ((n-p) \bmod{D}) - ((n-1) \bmod{D})) - 1 \equiv (p \bmod{D}) - ((n-1) \bmod{D})) + n - 1 \pmod{\ordn{2}}
\end{align}

We may simplify further using the fact that $((n-1) \bmod{D}) = D-1$:
\begin{align}
...
By the properties of the multiplicative order modulo $n$, $n \equiv 1 \pmod{\ordn{2}}$. We apply this to see:
\begin{align}
    ((n-p) \bmod{D}) - (p \bmod{D})\equiv 1 \pmod{\ordn{2}}
\end{align}

The congruence above implies that $n \equiv 0 \pmod{D}$, hence $D$ must divide $n$. However, this contradicts our requirement that $n$ not have a prime divisor $\leq D+1$. Therefore, we may conclude:
\begin{align}
...
\begin{center}
    \begin{enumerate}
        ...
        \item If $n$ has a prime factor $\leq D + 1$, output COMPOSITE.
...
\begin{enumerate}
    ...
    \item \textbf{Checking for a prime factor \(\leq D+1 \)}:
    
        Checking all integers \( \leq D+1 \) to verify they do not divide \( n \) takes at most \( O(\log(n)) \) steps, with each step requiring \( O(1) \) time for the mod operation. Hence, the overall complexity is \( O(\log(n)) \).
...
\Function{IsPrime}{$n$}
    ...
    \For{$i \gets 3$ \textbf{to} $\Call{Max}{log, 3}$} \Comment{Find $D$, the least non-divisor of $n-1$}
        ...
        \EndIf
    \EndFor
    \For{$i \gets 3$ \textbf{to} $D+1$} \Comment{Check for divisibility of $n$ up to $D+1$}
