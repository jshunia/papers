...
If $n$ is an integer $> 3$, then there exists an integer $1 < D \leq \left\lceil\log_2(n-1)\right\rceil$ which does not divide $n-1$.
\end{lemma}
\begin{proof}
Let $n$ be a composite integer $> 3$. Clearly, $n-1$ can have at most $\log_2(n-1)$ prime factors (when $n-1$ is a power of $2$). Hence, there must exist a $D \leq \left\lceil\log_2(n-1)\right\rceil$ which does not divide $n-1$.
\end{proof}
