...
\subsection{Binomial Coefficient Formula Proof}
\begin{thm}
\label{thm:2}
\begin{equation}
    \binom{n}{k} = \left\lfloor\frac{(1 + 2^{n})^{n}}{2^{n k}}\right\rfloor \bmod{2^{n}} \text{,} \quad \text{for } n > 0 \text{ and } 0 \leq k \leq n
...
\end{enumerate}
Therefore, by carefully selecting the base for the Kronecker substitution, we arrive at the formula presented in Theorem \ref{thm:2}. While Kronecker substitution is a well-known technique in computation, the author was unable to find any existing literature in which it is used to derive an explicit formula for binomial coefficients.
...
\section{Formula for Coefficients in the Multinomial Expansion of Univariate Unit Polynomials}
...
\section{The Problem of Efficiently Computing Binomial Coefficients Modulo n}
...
\begin{align*}
    \binom{n}{k} &\equiv \frac{(1 + 2^{n})^{n}}{2^{nk}} - \frac{((1 + 2^{n})^{n} \bmod{2^{nk}})}{2^{nk}}\\&- 2^{n} \frac{(1 + 2^{n})^{n}}{2^{nk+n}} - \frac{((1 + 2^{n})^{n} \bmod{2^{nk+n}})}{2^{nk+n}} \pmod{n}
\end{align*}

We can distribute the mod operation for $n$ and use modular exponentiation to efficiently compute the values for the majority of the terms modulo $n$. The only terms for which we cannot efficiently compute this way are:
\begin{align}
...
If we could compute the above terms efficiently, then we could calculate \( \binom{n}{k} \) modulo \( n \) efficiently as well. This is a rather interesting problem of evaluating a double mod operation involving large integer integer powers and \( n \). We leave the generalized version of this as an open problem.

\textbf{Problem Statement:}
Determine an efficient algorithm or mathematical approach for calculating:
\begin{align*}
    \left(a^{j} \bmod{b^k}\right) \bmod{n},\quad \text{where } n > 1\text{, } a > 1\text{, } b > 1\text{, } \text{ and } a^j > b^k > n
\end{align*}
Doing so would enable efficient computation of binomial coefficients modulo \( n \) using a version of our formula.
