...
\item $k < 0$: In this case, we have $(D+1)^{-nk} = \frac{1}{(D+1)^{nk}}$ and the original expression becomes $(D+1)^{nk} \cdot \frac{((D + 1)^{n D + n} - 1)}{((D + 1)^{n} - 1))^{n}} \bmod{(D+1)^n}$, hence the value of the expression is always equal to $0$ since $(D+1)^{n} \mid (D+1)^{nk}$.
