...
\date{September 2023 \\ \small (Revised: August 2024, Version 8) \normalsize}
...
\end{align*}
Since $b \geq 1$, the coefficients of $a_{k-1} f(b)^{-1} + \cdots + a_0 f(b)^{-k}$ will sum to a value that is less than $1$. The next step is to take the floor of the quotient $\frac{f(f(b))}{f(b)^k}$ to isolate the terms ranging from $a_k x^k$ up to and including $a_r x^r$. The result is
\begin{align*}
...
&= a_k \bmod{f(b)} .
\end{align*}
By assumption, all coefficients of $f(x)$ are non-negative. Furthermore, since $b \geq 1$ and $\deg(f(x)) = r \geq 1$, we know that $f(b) > a_k$. Therefore, the modular reduction by $f(b)$ leaves the coefficient $a_k$ unchanged. Thus, we arrive at
\begin{align*}
