...
\section[The Problem of Efficiently Calculating n choose k Modulo n]{The Problem of Efficiently Calculating\\ \( \binom{n}{k} \) Modulo \( n \)}
...
Determine an algorithm with time complexity \( O(\log^{c}{n}) \) for calculating:
\begin{align*}
...
\end{align*}
Solving this problem will enable the calculation of binomial coefficients modulo \( n \) in \( O(\log^{c}{n}) \) time using a variant of our formula.
\end{mdframed}
