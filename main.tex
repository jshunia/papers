...
\section{Generalization to Coefficients in Multinomial Expansion}
\subsection{Notation for Polynomial Coefficients}
We introduce a simplified notation to represent coefficients in the expansion of univariate polynomials. To denote the coefficient of the term with degree \( k \) in the polynomial expansion of \( (1 + x + \ldots + x^D)^n \), we write:
...
\subsection{Generalization of the Binomial Coefficient Formula}
...
Therefore, we have:
\begin{equation}
P(P(1,D)) = (1 + (D+1)^n + \ldots + (D+1)^{n D})^n
\end{equation}
Observe that the inner sum is equivalent to the summation of the powers $(D+1)^n$ from $0$ to $D$. We note the following identity \cite{A023037}:
\begin{equation}
\sum_{k=0}^{n-1} n^k = \frac{n^{n} - 1}{n - 1}
\end{equation}
By substitution, we have:
\begin{equation}
P(P(1,D))^n = \left(\sum_{k=0}^{D} (D+1)^{nk}\right)^n = \left(\frac{(D+1)^{nD+n} - 1}{(D+1)^{n} - 1}\right)^n
\end{equation}
In of our proof of Theorem \ref{thm:4}, we established that:
\begin{equation}
\binom{n}{k} = \left\lfloor\frac{P(P(1))^{n}}{P(1)^{n k}}\right\rfloor \bmod{P(1)^{n}}
\end{equation}
By equivalence, we have:
\begin{equation}
\underline{D}\binom{n}{k} = \left\lfloor\frac{P(P(1,D))^{n}}{P(1,D)^{n k}}\right\rfloor \bmod{P(1,D)^{n}}
\end{equation}
Finally, we replace the values of $P(1,D)^n$ and $P(P(1,D),D)^n$ to arrive at our original equation:
\begin{equation}
...
\item $k < 0$: In this case, we have $(D+1)^{-nk} = \frac{1}{(D+1)^{nk}}$ and the original expression becomes $(D+1)^{nk} \cdot \frac{((D + 1)^{n D + n} - 1)}{((D + 1)^{n} - 1))^{n}} \bmod{(D+1)^n}$, hence the value of our equation is always equal to $0$ since $(D+1)^{n} \mid (D+1)^{nk}$.

\item $k > nD$: In this case, $(D+1)^{nk} > \frac{((D + 1)^{n D + n} - 1)}{((D + 1)^{n} - 1))^{n}} > (D+1)^{nn}$, which is always equal to $0$.
\end{enumerate}
\noindent
\textbf{Conclusion:}
\\
...
\end{align*}
\\
Where \(\underline{D}\binom{n}{k}\) denotes the coefficient of the term with degree \(k\) in the expansion of \((1+x+\ldots+x^D)^n\). This generalizes our binomial formula to multinomials and represents the first known general closed form expression for such coefficients.
\\
\\
